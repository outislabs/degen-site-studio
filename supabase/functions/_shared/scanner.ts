// Shared token security scanning — used by sniper-security-scan and cli-scan.

export function computeSafetyScore(security: any): { score: number; level: string } {
  let score = 100;
...
export async function scanSolanaToken(address: string): Promise<any> {
  try {
    ...
    const gp =
      goplusData?.result?.[address.toLowerCase()] ??
      goplusData?.result?.[address] ??
      null;
    ...
    const topHolderPct = topHolders
      .slice(0, 10)
      .reduce(
        (sum: number, h: any) => sum + parseFloat(h.percent ?? "0"),
        0,
      ) * 100;
    ...
    return {
      ...
      contract_renounced:
        gp?.owner_address === "" || gp?.owner_address === null,
      lp_locked: lpLocked,
    ...
    console.error(`Error scanning Solana token ${address}:`, err);
    return null;
  }
}
...
export async function scanEVMToken(
  ...
  try {
    ...
    return {
      ...
      creator_address: gp?.creator_address ?? null,
      contract_renounced:
        gp?.owner_address === "" || gp?.renounced === "1",
      lp_locked: lpLocked,
      lp_lock_pct: lpLockPct,
      top_holder_pct: topHolderPct,
      dev_holding_pct: devHoldingPct,
    ...
    console.error(`Error scanning EVM token ${address} on ${chainId}:`, err);
    return null;
  }
}
