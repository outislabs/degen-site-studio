...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create_fee_config") {
      ...
      // Correct body format: claimersArray + basisPointsArray
      // Creator gets 100% of user-side fees
      // Partner key earns its share separately via the partner/partnerConfig params
      ...
      // Embed DegenTools partner key — earns 25% (or negotiated %) of protocol fees
      ...
      // Extract transaction strings from response objects
      // Each transaction entry has { blockhash: {...}, transaction: "base58string" }
