...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'list') {
      ...
      const page = Math.max(1, Math.min(100, parseInt(body.page ?? url.searchParams.get('page') ?? '1') || 1))
      const perPage = Math.max(1, Math.min(1000, parseInt(body.per_page ?? url.searchParams.get('per_page') ?? '50') || 50))
